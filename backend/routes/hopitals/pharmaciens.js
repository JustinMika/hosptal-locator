...
router.post('/avec-ordonance/', async (req, res) => {
    try {
        const { nom, telephone, withLivraison, step, file, adresse } = req.body
        if (!nom && !telephone) {
            res.status(402).json({
                message: 'Veuillez remplir tous les champs svp, ...',
                status: error.status
        ...
        if (rows.length === 0) {
            ...
            const ordonance = `INSERT INTO ordonances (client_id, file_ordonance, uuid, isLivraisonAtHome, adresse, created_at, updated_at) VALUES('${client_id}', '${filePath}', '${uuid}', ${withLivraison}, '${adresse}', '${formattedDate}', '${formattedDate}')`
...
router.post('/sans-ordonance/', async (req, res) => {
    try {
        ...
        if (rows.length === 0) {
            ...
            const client_id = rows[0]['id']
            const q = `INSERT INTO symptomes_clients (client_id, Livrable, symptome, isPregnant, sex, age, weight, adresse, uuid, created_at, updated_at) VALUES (?,?,?,?,?,?,?,?,?,?,?)`
            const Value = [
                client_id,
            ...
            const client_id = rows[0]['id']
            const q = `INSERT INTO symptomes_clients (client_id, Livrable, symptome, isPregnant, sex, age, weight, uuid, adresse, created_at, updated_at) VALUES (?,?,?,?,?,?,?,?,?,?,?)`
            const Value = [
                client_id,
                Livrable == '1' ? true : false,
                symptome,
        ...
        saveErrors(error.message ?? '-', 'Pharmanciens.js', error.status ?? 0)
        res.status(500).json({
            message: `une erreur s'est produite` ?? 'Erreur'
        })
    }
})
...
router.get(
    ...
    async (req, res) => {
        try {
            ...
            const query = `SELECT * FROM stock_medocs WHERE user_id=${id_my_id} ORDER BY created_at DESC`
...
router.get(
    ...
    async (req, res) => {
        try {
            ...
            const query = `SELECT * FROM stock_medocs WHERE id=${id_my_id} ORDER BY created_at DESC`
            const data = await connection.query(query)
            res.status(200).json({
                message: 'Données récuperées avec succès',
                status: 'success',
...
router.put(
    ...
    async (req, res) => {
        try {
            ...
            const query = `UPDATE stock_medocs SET medicament='${medicament}', quantite='${quantite}', price='${price}', devise='${devise}' WHERE id=${id}`
            const d = await connection.query(query)

            res.status(201).json({
                message: 'medicament info updated successfully',
            })
        } catch (error) {
            if (error) {
                saveErrors(
                ...
                res.status(401).json({
                    message: `Une erreur "${error?.message}" s'est produite`,
                    status: error.status
                })
            }
        }
...
router.put(
    ...
    async (req, res) => {
        try {
            ...
            const query = `UPDATE stock_medocs SET photo_medicament='${filePath}' WHERE id=${user_id}`
...
router.delete(
    ...
    async (req, res) => {
        try {
            ...
            if (!id_my_id) {
                res.status(403).json({
                    message: 'Veuillez selectionner un element svp.',
                    status: 'failled'
                })
            }
            const query = `DELETE FROM stock_medocs WHERE id=${id_my_id}`
            await connection.query(query)
            res.status(200).json({
                message: 'Données DELETE avec succès',
                status: 'success',
            })
        } catch (error) {
            if (error) {
...
router.post('/medicaments/', verifyCsrfToken, verifyUser, async (req, res) => {
    try {
        const { medicament, quantite, file_medicament, id, price, devise } = req.body

        if (!medicament && !quantite && !file_medicament && !id && !price && !devise) {
            res.status(404).json({
                message: 'Veuillez remplir tous les champs svp!',
                status: 'error'
            })
        ...
        const query = `INSERT INTO stock_medocs (user_id, medicament, quantite, price, devise, photo_medicament, uuid, created_at, updated_at) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)`
